...
// Ключ DEEPSEEK_API_KEY берётся из окружения сервера и НИКОГДА не попадает
// в клиент. Клиент шлёт только тему/уровень/кол-во.
//
// Настройка: ключ живёт в /opt/precettore-web/api.env на сервере, ставится
// скриптом `/usr/local/sbin/set-deepseek-key.sh` (ключ на stdin) — он же
// пересоздаёт контейнер, потому что `docker restart` env_file не перечитывает.
//
...
// С включёнными «размышлениями» генерация занимает 10–60 с. Наследие Vercel:
// там этот экспорт поднимал лимит функции. У нас таймауты задаёт server/index.js,
// и число осталось как запись ожидаемой длительности — оттуда на него и ссылка.
...
// памяти процесса, а тот перезапускается при каждом деплое, поэтому это не
// строгий барьер, а защита от самого дешёвого злоупотребления. Строгий лимит
// потребует внешнего хранилища (KV/Redis) — отдельным шагом.
...
// прокси ПЕРЕЗАПИСЫВАЕТ поверх присланного: `x-real-ip`, его задаёт
// `header_up X-Real-IP {remote_host}` в Caddyfile (server/Caddyfile.precettore)
// — правки здесь и там парные.
// XFF убран из цепочки сознательно: за Caddy настоящий адрес дописывается в
// КОНЕЦ списка, так что старое «взять первый» отдавало бы подделку. Ровно по
// этой причине 09.09.2026 убран и `x-vercel-forwarded-for`: его перезаписывал
// прокси Vercel, а после ухода туда (проект удалён) заголовок стал обычным
// клиентским — и, стоя в цепочке ПЕРВЫМ, позволял прислать любой адрес поверх
// честного `x-real-ip`. Последний запасной вариант — адрес сокета: он не
// подделывается, а если прокси однажды перестанет слать заголовок, лимит
// просто станет общим на всех вместо того, чтобы пускать вебхук с любого адреса.
