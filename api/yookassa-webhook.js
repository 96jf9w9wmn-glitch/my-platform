...
// сервер живёт в UTC, и вечерний платёж иначе уезжал бы на день назад.
function ruDate(iso) {
...
export default async function handler(req, res) {
  ...
  // поэтому берём его только из заголовка, который ПЕРЕЗАПИСЫВАЕТ наш Caddy
  // (`X-Real-IP`): клиентский `x-forwarded-for` подделывается и пускал бы кого
  // угодно (аудит 30.07.2026). См. clientIp в api/generate-hw.js.
