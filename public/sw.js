...
//
// v6 (15.09.2026): движок питона (/py/<версия>/…, 13 МБ) переведён в
// cache-first. Версия стоит В ПУТИ, поэтому файл по такому адресу неизменен,
// как ассет с хэшем. Оставить его в общей ветке «network-first» было нельзя:
// каждый запуск программы означал бы поход в сеть за мегабайтами, а копия всё
// равно оседала бы в кэше.
const CACHE = "precettore-v6"
...
// Туда же файлы питона: /py/314.0.7/pyodide.asm.wasm — версия в самом пути,
// поэтому содержимое по этому адресу не меняется никогда.
function isImmutableAsset(url) {
  return url.pathname.startsWith("/assets/") || url.pathname.startsWith("/py/")
}
