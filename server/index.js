...
// ровно этот сахар и зовём тот же `export default`. Поэтому при переезде файлы
// в `api/` не правились вовсе, и переключение сводилось к смене DNS, а не к
// правке кода. Сам проект на Vercel удалён 09.09.2026 — контракт остался просто
// потому, что он обычный node-шный и переписывать его незачем.
//
