...
export async function sendPush(subscription, payload, vapid, { ttl = 86400, urgency = "normal" } = {}) {
  ...
  // 404/410 — подписки нет. 403 — наш ключ не тот, на который подписывались:
  // это не про конкретное устройство, а про ротацию VAPID, поэтому чистить по
  // нему всё подряд нельзя.
