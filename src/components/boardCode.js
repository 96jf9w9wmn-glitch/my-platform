...
// Поля карточки в единицах кегля. Формула ОДНА на всех: её берут и метрики
// штриха (textMetrics), и слой подсветки под полем ввода — разойдутся, и
// набираемый код поедет относительно того, что нарисовано под ним.
export const codePad = (size) => Math.max(1, size || 1) * CODE_PAD_K
...
// Один ответ на вопрос «класть ли карточкой»: огороженный блок — да, иначе по
// признакам. Зовут и обработчик вставки, и сама вставка, поэтому правило живёт
// в одном месте, а не переписывается на каждой стороне.
export function isCodeText(raw) {
  if (readFence(raw)) return true
  return looksLikeCode(normalizeCode(raw))
}
