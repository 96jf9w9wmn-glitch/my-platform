...
//
// Выпрямляем РОВНО ЧЕТЫРЕ вещи: прямую, круг, квадрат и прямоугольник. Всё
// остальное — треугольник, ромб, трапеция, произвольный контур — остаётся
// рукописным: там подмена меняет сам чертёж (прямоугольный треугольник стал бы
// равнобедренным), а это хуже неровной линии.
// Возвращает готовую фигуру { tool, a, b } — габарит, как у инструмента «Фигуры».
...
export function recognizeShape(rawPoints, { minSize = 30 } = {}) {
  ...
  // Углов не четыре — треугольник, пятиугольник и прочее. Такое SmartDraw не
  // выпрямляет: штрих остаётся рукописным.
  if (n !== 4) return null
