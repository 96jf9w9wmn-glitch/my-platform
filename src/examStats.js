...
//
// ЦЕЛЬ БЫВАЕТ НЕ ЭКЗАМЕНОМ. «Успеваемость» — такая же цель, как ОГЭ и ЕГЭ, но
// предмета с таким ключом нет ни одного, и, подставленная сюда, она отсекала
// РОВНО ВСЕ попытки ученика: раздел «Результаты» у семиклассника оставался
// пустым при полном журнале. Экзаменом считаем только ОГЭ и ЕГЭ, остальное —
// «предмет неизвестен» (пустая строка), и тогда попытки не фильтруются вовсе.
const EXAM_GOALS = new Set(["ОГЭ", "ЕГЭ"])
export const examTypeOf = (student, stats) =>
  stats.rows?.[stats.rows.length - 1]?.type || student.examType ||
  (EXAM_GOALS.has(student.goal) ? student.goal : "") || ""
