...
//
// Отсюда следствие, о котором легко забыть: удаление строки группы само по
// себе занятий НЕ трогает — они уже стоят в расписании у людей. Поэтому при
// роспуске репетитор ВЫБИРАЕТ (окно группы), снимать ли будущие занятия;
// снимает их кабинет тем же `applyGroupSchedule` с пустым расписанием, каким
// теряет занятия исключённый участник. Прошлое не трогается никогда: оно
// проведено, посчитано в долг и выставлено квитанциями, а метка `groupId` у
// него остаётся — занятие показывается групповым по снимку названия в самом
// занятии (`groupName`).
