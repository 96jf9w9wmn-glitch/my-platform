...
//
// Задание попадает в журнал, когда у него есть ПРЕДМЕТ и хоть одна разбивка —
// НОМЕР экзамена ИЛИ ТЕМА. У собранного из банка есть и то и другое сразу, у
// нарезанного из своего файла их проставляет репетитор (поля «№» и «Тема»).
//
// НОМЕР НЕОБЯЗАТЕЛЕН НАМЕРЕННО: к экзамену готовятся не все. У семиклассника,
// подтягивающего успеваемость, номера нет и быть не может, и пока журнал
// требовал его, такой ученик не попадал в статистику ни одним заданием — ему
// нельзя было сказать даже, над чем работать. Разбивку в этом случае держит
// тема («theme:Квадратные уравнения»), и строка считается только по ней: в
// карту заданий она не идёт, та про номера экзамена.
//
// Нет ни номера, ни темы — задание в журнал не идёт; работа при этом
// проверяется и оценивается как обычно.
//
...
export function homeworkAttempts(hw, { answers, correct, account, token, studentId }) {
  ...
  ;(answers || []).forEach((ans, i) => {
    ...
    const task = bank[i]
    const genKey = taskGenKey(task)
    // Задание, которое ничем не размечено (работа из файла, которую не тронули),
    // в журнал не идёт: такую строку нечем отличить от соседней.
    if (!task?.exam_type || (task.number == null && !genKey)) return
    ...
    out.push({
      ...
      // Номера может не быть вовсе — колонка это допускает (см. шапку файла).
      p_number: task.number == null ? null : Number(task.number),
      ...
      // номера все задания были бы на одно лицо, а без номера разбивки не было
      // бы вовсе. Нет ни того, ни другого — null: колонка это допускает («не из
      // генератора»), а своды по номерам такие строки считают наравне с
      // остальными.
      p_gen_key: genKey,
...
export function markupAttempts(hw) {
  ...
  bank.forEach((task, i) => {
    ...
    const genKey = taskGenKey(task)
    if (!task?.exam_type || (task.number == null && !genKey) || corr === "") return
    out.push({
      ...
      number: task.number == null ? null : Number(task.number),
      gen_key: genKey,
