...
function StudentDetail({ student, stats, hw, tutorId }) {
  ...
  // Карта — про НОМЕРА экзамена, и ученику, который к экзамену не готовится,
  // она показывала бы пустую сетку из двадцати пяти клеток: его задания
  // размечены темой, а номера у них нет. Но пустая цель («не указана») экзамену
  // не противоречит — у половины боевых карточек она пуста, — поэтому карта
  // убирается, только когда цель НЕ экзамен И ни одной попытки с номером нет.
  const numbered = (attempts || []).some((a) => a.number != null)
  const taskMap = stats.isExam || !attempts || numbered ? (
          ...
          : (
            <div className="px-3.5 pb-3.5 sm:px-4 sm:pb-4 flex flex-col gap-3">
              <div className="glass-sm p-4 text-sm text-gray-500">
                {stats.isExam
                  ? "Баллы появятся, когда ученик решит вариант и вы его проверите. Решённые домашние работы встанут здесь отдельной дорожкой."
                  : "Здесь появятся решённые домашние работы и задания, в которых ученик чаще ошибается."}
              </div>
