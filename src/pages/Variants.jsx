...
// Банк заданий — самый тяжёлый модуль приложения: генераторы всех предметов весят
// 4 МБ, и одно их ПОДКЛЮЧЕНИЕ (разбор и исполнение модуля) занимает главный поток
// на 2,8 с — замерено на боевом сайте 15.09.2026. Всё это время браузер не рисует
// ни кадра: кабинет стоит, нажатия копятся, раздел въезжает от прозрачного и на
// экране пусто. Поэтому банк подключается ТОЛЬКО по нажатию («Собрать вариант»),
// где ожидание видно и его ждут. Фоновый прогрев «сразу после отрисовки» тут
...
// Подпись у ученика, чья цель занятий расходится с уровнем варианта. Цель —
// не всегда экзамен: кроме «ОГЭ» и «ЕГЭ» её значением бывает «Успеваемость»,
// и подстановка в «готовится к …» давала «готовится к Успеваемость».
function goalNote(goal, examLevel) {
  if (!goal || goal === examLevel) return ""
  if (goal === "ОГЭ" || goal === "ЕГЭ") return `готовится к ${goal}`
  return `цель занятий — ${goal.toLowerCase()}`
}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4 items-stretch">
            <div className="flex flex-col gap-4">
              ...
              <StudentPicker
                ...
                // Цель бывает не только экзаменом: «Успеваемость» в эту
                // формулировку не встаёт («готовится к Успеваемость»).
                noteOf={(a) => goalNote(goalByAccountId[a.id], examLevel)}
              />
  ...
  return (
    <div className="flex flex-col gap-5 p-4 md:p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div>
          <h1 className="text-2xl font-medium page-title">Варианты</h1>
          <p className="text-[15px] page-subtitle mt-0.5">Пробные варианты ОГЭ и ЕГЭ: соберите из банка заданий или приложите свой файл — ученик решит вариант в кабинете.</p>
        </div>
