...
export function detectStarts(lines, pageHeight = 0, loose = false) {
  ...
  for (const l of lines) {
    ...
    // Строка ИЗ ОДНОГО числа — почти всегда номер задания: предложение с
    // такой строки не начинается. Без этого случая разбор проваливался на
    // раздатке, где номер стоит отдельной строкой без точки и ровно по тому же
    // левому полю, что и условие (так собран лист, выгруженный из курса):
    // номера не признавались, и работа резалась по пустым полосам — с потерей
    // первого задания. Исключение — номер страницы: он стоит в самом низу.
    // Строка ИЗ ОДНОГО числа у левого поля: так номер стоит в листах,
    // выгруженных из курсов. Правее поля не берём — там такие строки обычно
    // ячейки таблицы, а не номера; в самом низу страницы — колонтитул.
    const alone = loose && /^\d{1,3}$/.test(l.text.trim()) &&
      (left == null || l.x <= left + 2) && (!pageHeight || l.y < pageHeight * 0.92)
    const explicit = alone || /^(?:задание|задача|упражнение|упр\.?|[№#])/i.test(l.text) || /^\d{1,3}\s*[.)]/.test(l.text)
...
export function chainStarts(all) {
  ...
  // Раздатка часто состоит из разделов, и в каждом нумерация начинается снова
  // («квадратные уравнения» 1…10, потом «вынос общего» 1…7). Одной возрастающей
  // цепочкой такой документ режется по первому разделу, а остальные пропадают,
  // поэтому режем кандидатов на куски по каждому возврату к единице и ищем
  // цепочку внутри куска. Возврат к единице после одного-единственного номера
  // разделом не считаем — это скорее случайное число.
  const segs = []
  let cur = []
  for (const c of all) {
    if (c.n === 1 && cur.some((x) => x.n >= 2)) { segs.push(cur); cur = [] }
    cur.push(c)
  }
  segs.push(cur)
  if (segs.length > 1) return segs.flatMap((seg) => chainStarts(seg))
...
export async function splitSource(file, onProgress) {
  ...
  const byText = (loose) => {
    ...
    pages.forEach((p, page) =>
      detectStarts(p.lines, p.height, loose).forEach((s) => all.push({ ...s, page })))
    return spaced(chainStarts(all), pages[0]?.height || 1750)
  }
  // Сначала номер с точкой, скобкой или словом «Задание» — так он выглядит в
  // раздатках, и спутать его с числом из условия почти невозможно. Если такого
  // нет, пробуем строку, состоящую ИЗ ОДНОГО числа: так номер стоит в листах,
  // выгруженных из курсов, и без этого захода они резались по пустым полосам —
  // с потерей первого задания.
  // Мягкий разбор берём не «если строгий пустой», а когда он находит заметно
  // больше: строгому хватает пары случайных чисел в условиях, чтобы вернуть
  // короткий список и закрыть дорогу точному разбору (проверено на листе из
  // 76 заданий — строгий давал 9 ложных границ, мягкий 66 настоящих).
  const strict = byText(false)
  const loose = byText(true)
  let starts = loose.length > strict.length * 1.5 ? loose : strict
  ...
  if (starts.length < 2) {
    ...
    // Кусок берётся ОТ границы до следующей, поэтому всё, что выше первой,
    // в задания не попадает. Пустая полоса стоит между заданиями, а не перед
    // первым — без границы в начале страницы первое задание просто пропадало
    // (проверено: из девяти заданий выдавалось восемь, работа начиналась со
    // второго). Ставим её сами, если сверху осталось больше узкой шапки: лишний
    // кусок с заголовком видно в редакторе и он убирается крестиком, а
    // потерянного задания не видно вовсе.
    const first = starts[0]
    if (starts.length && (first.page > 0 || first.y > (pages[0]?.height || 0) * 0.04)) {
      starts.unshift({ page: 0, y: 0, guess: true })
    }
  }
