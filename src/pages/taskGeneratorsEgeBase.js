...
// PF — дробь В СКОБКАХ по её высоте: основание степени всегда в скобках, (1/5)^(2x−6).
const PF = (num, den) => `⟦pf:${num}:${den}⟧`
const rIn = (x) => `√{${x}}`
...
function t17Exponential() {
  for (let t = 0; t < 200; t++) {
    ...
    const base = recip ? PF("1", String(b)) : String(b)   // дробь-основание — в скобках
...
// Сплавы: m1·p1 + m2·p2 = (m1+m2)·p3 → m2 = m1(p3−p1)/(p2−p3).
function t20Alloy() {
  for (let t = 0; t < 400; t++) {
    const p1 = pick([5, 10, 15, 20, 25, 30, 35]), p2 = pick([40, 45, 50, 55, 60, 70]), p3 = randInt(p1 + 5, p2 - 5)
    const m1 = randInt(2, 12) * 5
    const num = m1 * (p3 - p1), den = p2 - p3
    if (num % den) continue
    const m2 = num / den
    if (m2 <= 0 || m2 > 400) continue
    const q = pick(["diff", "mass2"])
    if (q === "diff" && m2 <= m1) continue
    return {
      condition_text: `Имеется два сплава. Первый содержит ${p1} % никеля, второй — ${p2} % никеля. ` +
        `Из этих двух сплавов получили третий сплав, содержащий ${p3} % никеля. Масса первого ` +
        `сплава равна ${m1} кг. ` +
        (q === "diff"
          ? `На сколько килограммов масса первого сплава была меньше массы второго?`
          : `Найдите массу второго сплава. Ответ дайте в килограммах.`),
      answer: ru(q === "diff" ? m2 - m1 : m2),
    }
  }
  return { condition_text: `Имеется два сплава. Первый содержит 35 % никеля, второй — 50 % никеля. Из этих двух сплавов получили третий сплав, содержащий 45 % никеля. Масса первого сплава равна 35 кг. Найдите массу второго сплава. Ответ дайте в килограммах.`, answer: "70" }
}

// Банковский вклад: S = A(1 + p/100)^n.
function t20Deposit() {
  const A = randInt(10, 60) * 1000, p = pick([5, 10, 20, 25]), n = pick([1, 2])
  const S = A * (1 + p / 100) ** n
  return {
    condition_text: `Для определения итоговой суммы вклада используется формула S = A · (1 + ` +
      `${FF("p", "100")})${supT("n")}, где A — первоначальная сумма (в рублях), p — годовая ставка ` +
      `(в процентах), n — срок (в годах). Пользуясь формулой, найдите S (в рублях), если A = ${A} ` +
      `рублей, p = ${p} % и n = ${n} ${plural(n, "год", "года", "лет")}.`,
    answer: ru(Math.round(S)),
  }
}

// Дорога подъём+спуск: v·t + (v−d)(T−t) = L → v = (L + d(T−t))/T (скорость на спуске).
function t20UpDown() {
  for (let t = 0; t < 400; t++) {
    const T = randInt(5, 14), tDesc = randInt(2, T - 1), d = randInt(1, 3), L = randInt(15, 90)
    const num = L + d * (T - tDesc)
    if (num % T) continue
    const v = num / T
    if (v - d < 1 || v > 60) continue
    if ((v * tDesc + (v - d) * (T - tDesc)) !== L) continue
    return {
      condition_text: `Дорога между пунктами А и В состоит из подъёма и спуска, а её длина равна ${L} км. ` +
        `Путь из А в В занял у туриста ${T} часов, из них ${tDesc} часов ушло на спуск. Найдите ` +
        `скорость туриста на спуске, если она больше скорости на подъёме на ${d} км/ч. Ответ дайте в км/ч.`,
      answer: ru(v),
    }
  }
  return { condition_text: `Дорога между пунктами А и В состоит из подъёма и спуска, а её длина равна 30 км. Путь из А в В занял у туриста 11 часов, из них 8 часов ушло на спуск. Найдите скорость туриста на спуске, если она больше скорости на подъёме на 1 км/ч. Ответ дайте в км/ч.`, answer: "3" }
}

// Моторная лодка (течение): S/(x−c) − S/(x+c) = h → x = √(c² + 2Sc/h).
function t20Boat() {
  for (let t = 0; t < 500; t++) {
    const c = randInt(1, 4), h = pick([1, 2, 3]), S = randInt(20, 60) * pick([2, 3, 4, 5])
    const inside = c * c + (2 * S * c) / h
    const x = Math.sqrt(inside)
    if (!Number.isInteger(x) || x <= c || x > 60) continue
    return {
      condition_text: `Моторная лодка прошла против течения реки ${S} км и вернулась в пункт ` +
        `отправления, затратив на обратный путь на ${h} ${plural(h, "час", "часа", "часов")} меньше. ` +
        `Найдите скорость лодки в неподвижной воде, если скорость течения равна ${c} км/ч. Ответ дайте в км/ч.`,
      answer: ru(x),
    }
  }
  return { condition_text: `Моторная лодка прошла против течения реки 224 км и вернулась в пункт отправления, затратив на обратный путь на 2 часа меньше. Найдите скорость лодки в неподвижной воде, если скорость течения равна 1 км/ч. Ответ дайте в км/ч.`, answer: "15" }
}

// Одинаковый тест: N/a − N/b = Δ/60 → N = Δ·a·b/(60(b−a)).
function t20Test() {
  for (let t = 0; t < 400; t++) {
    const a = randInt(8, 18), b = a + randInt(1, 6), dm = pick([20, 30, 40, 45, 60])
    const num = dm * a * b, den = 60 * (b - a)
    if (num % den) continue
    const N = num / den
    if (N < 20 || N > 200) continue
    const names = pick([["Петя", "Ваня"], ["Коля", "Вася"], ["Саша", "Дима"]])
    return {
      condition_text: `${names[0]} и ${names[1]} выполняют одинаковый тест. ${names[0]} отвечает за ` +
        `час на ${a} вопросов теста, а ${names[1]} — на ${b}. Они одновременно начали отвечать, и ` +
        `${names[0]} закончил свой тест позже ${names[1] === "Ваня" ? "Вани" : names[1] + "и"} на ${dm} минут. ` +
        `Сколько вопросов содержит тест?`,
      answer: ru(N),
    }
  }
  return { condition_text: `Петя и Ваня выполняют одинаковый тест. Петя отвечает за час на 13 вопросов теста, а Ваня — на 15. Они одновременно начали отвечать, и Петя закончил свой тест позже Вани на 40 минут. Сколько вопросов содержит тест?`, answer: "65" }
}

// Пешеход и велосипедист навстречу: (v + (v+d))·t = L → v = (L/t − d)/2.
function t20PedCyclist() {
  for (let t = 0; t < 200; t++) {
    const time = randInt(2, 4), d = pick([6, 8, 10, 12]), v = randInt(3, 12)
    const L = (2 * v + d) * time
    return {
      condition_text: `Пешеход и велосипедист одновременно отправились из посёлков, расстояние между ` +
        `которыми ${L} км, навстречу друг другу. Они встретились через ${time} ` +
        `${plural(time, "час", "часа", "часов")}. Скорость велосипедиста на ${d} км/ч больше скорости ` +
        `пешехода. Найдите скорость пешехода. Ответ дайте в км/ч.`,
      answer: ru(v),
    }
  }
  return { condition_text: `Пешеход и велосипедист одновременно отправились из посёлков, расстояние между которыми 36 км, навстречу друг другу. Они встретились через 2 часа. Скорость велосипедиста на 8 км/ч больше скорости пешехода. Найдите скорость пешехода. Ответ дайте в км/ч.`, answer: "5" }
}
...
// Среднее (n+1)-го числа: avg(n)=m, avg(n+1)=M → x = (n+1)M − n·m.
function t21AvgFifth() {
  const NUM = { 3: "трёх", 4: "четырёх", 5: "пяти", 6: "шести", 7: "семи", 8: "восьми" }
  const ORD = { 4: "четвёртое", 5: "пятое", 6: "шестое", 7: "седьмое", 8: "восьмое", 9: "девятое" }
  const n = randInt(3, 8), m = randInt(6, 15), M = m + randInt(1, 5)
  return {
    condition_text: `Среднее арифметическое ${NUM[n]} различных натуральных чисел равно ${m}. ` +
      `Среднее арифметическое этих чисел и ${ORD[n + 1]} числа равно ${M}. Чему равно ${ORD[n + 1]} число?`,
    answer: ru((n + 1) * M - n * m),
  }
}

// Распил: pieces = cuts + boards → boards = pieces − cuts.
function t21Boards() {
  const boards = randInt(2, 9), cuts = randInt(boards + 2, boards + 16), pieces = cuts + boards
  return {
    condition_text: `Взяли несколько досок и распилили их. Всего сделали ${cuts} поперечных ` +
      `распилов, в итоге получилось ${pieces} кусков. Сколько досок взяли?`,
    answer: ru(boards),
  }
}

// Кузнечик: n прыжков ±1 из 0 → n+1 различных точек (та же чётность, шаг 2).
function t21Grasshopper() {
  const n = randInt(5, 25)
  return {
    condition_text: `Кузнечик прыгает вдоль координатной прямой в любом направлении на единичный ` +
      `отрезок за прыжок, делая первый прыжок из начала координат. Сколько существует различных ` +
      `точек на координатной прямой, в которых кузнечик может оказаться, совершив ровно ${n} ` +
      `${plural(n, "прыжок", "прыжка", "прыжков")}?`,
    answer: ru(n + 1),
  }
}

// Грибы: любые m1 → есть рыжик (груздей ≤ m1−1); любые m2 → есть груздь (рыжиков ≤ m2−1).
// N = (m1−1)+(m2−1) делает ответ единственным: груздей = m1−1.
function t21Mushrooms() {
  const G = randInt(8, 20), R = randInt(6, 15), N = G + R, m1 = G + 1, m2 = R + 1
  return {
    condition_text: `В корзине лежит ${N} грибов: рыжики и грузди. Известно, что среди любых ${m1} ` +
      `грибов имеется хотя бы один рыжик, а среди любых ${m2} грибов — хотя бы один груздь. ` +
      `Сколько груздей в корзине?`,
    answer: ru(G),
  }
}

// Лента: красная на расстоянии a от середины (разность частей 2a), синяя — b с другой стороны
// (2b). Расстояние между полосками = a + b = (d1 + d2)/2.
function t21Tape() {
  const a2 = 2 * randInt(2, 9), b2 = 2 * randInt(2, 9)   // разности (чётные)
  return {
    condition_text: `На ленте по разные стороны от её середины отмечены две тонкие поперечные ` +
      `полоски: синяя и красная. Если разрезать ленту по красной полоске, то одна часть будет на ` +
      `${a2} см длиннее другой. Если разрезать ленту по синей полоске, то одна часть будет на ${b2} ` +
      `см длиннее другой. Найдите расстояние между полосками (в сантиметрах).`,
    answer: ru((a2 + b2) / 2),
  }
}

// Таблица 3 столбца: сумма чисел в каждой строке ∈ (lo, lo+3) (т.е. lo+1 или lo+2); число строк
// R однозначно из lo+1 ≤ total/R ≤ lo+2 (total = сумма столбцов).
function t21Table() {
  for (let t = 0; t < 400; t++) {
    const R = randInt(8, 20), lo = randInt(20, 45)
    const total = randInt(R * (lo + 1), R * (lo + 2))
    let cnt = 0
    for (let r = 1; r <= 200; r++) if (r * (lo + 1) <= total && total <= r * (lo + 2)) cnt++
    if (cnt !== 1) continue
    const s1 = randInt(Math.floor(total / 3) - 8, Math.floor(total / 3) + 8)
    const s2 = randInt(Math.floor(total / 3) - 8, Math.floor(total / 3) + 8)
    const s3 = total - s1 - s2
    if (s3 < 1 || s1 < 1 || s2 < 1) continue
    return {
      condition_text: `В таблице три столбца и несколько строк. В каждую клетку таблицы вписали по ` +
        `натуральному числу так, что сумма всех чисел в первом столбце равна ${s1}, во втором — ${s2}, ` +
        `в третьем — ${s3}, а сумма чисел в каждой строке больше ${lo} и меньше ${lo + 3}. Сколько ` +
        `всего строк в таблице?`,
      answer: ru(R),
    }
  }
  return { condition_text: `В таблице три столбца и несколько строк. В каждую клетку вписали по натуральному числу так, что сумма чисел в первом столбце равна 124, во втором — 132, в третьем — 142, а сумма чисел в каждой строке больше 27 и меньше 30. Сколько всего строк в таблице?`, answer: "14" }
}

// Выпавшие листы: последняя страница L (чётная), первая после — перестановка цифр L, нечётная,
// больше L → выпало (F − L − 1)/2 листов.
function t21FallenPages() {
  for (let t = 0; t < 500; t++) {
    const L = randInt(100, 480) * 2 - randInt(0, 1) * 0   // сделаем чётной ниже
    const Le = L % 2 === 0 ? L : L + 1
    const perms = [...new Set(permsOf(String(Le)))].map(Number).filter((F) => F % 2 === 1 && F > Le && String(F).length === String(Le).length)
    if (!perms.length) continue
    const F = pick(perms)
    if ((F - Le - 1) % 2 !== 0) continue
    const leaves = (F - Le - 1) / 2
    if (leaves < 1 || leaves > 500) continue
    return {
      condition_text: `Из книги выпало несколько идущих подряд листов. Номер последней страницы ` +
        `перед выпавшими листами — ${Le}, номер первой страницы после выпавших листов записывается ` +
        `теми же цифрами, но в другом порядке. Сколько листов выпало?`,
      answer: ru(leaves),
    }
  }
  return { condition_text: `Из книги выпало несколько идущих подряд листов. Номер последней страницы перед выпавшими листами — 254, номер первой страницы после выпавших листов записывается теми же цифрами, но в другом порядке. Сколько листов выпало?`, answer: "85" }
}
function permsOf(s) {
  if (s.length <= 1) return [s]
  const out = []
  for (let i = 0; i < s.length; i++) for (const p of permsOf(s.slice(0, i) + s.slice(i + 1))) out.push(s[i] + p)
  return out
}

// Викторина: total вопросов, +a за верный, −b за неверный, 0 за пропуск; набрано S очков.
// Число пропусков z единственно (перебором проверяем).
function t21Quiz() {
  for (let t = 0; t < 400; t++) {
    const total = randInt(30, 70), a = randInt(5, 12), b = randInt(10, 20)
    const c = randInt(5, total - 5), w = randInt(0, total - c), z = total - c - w
    const S = a * c - b * w
    if (S <= 0) continue
    // единственность (c', w', z') с a c' − b w' = S, c'+w' ≤ total
    let sols = 0
    for (let cc = 0; cc <= total; cc++) { const ww = (a * cc - S) / b; if (Number.isInteger(ww) && ww >= 0 && cc + ww <= total) sols++ }
    if (sols !== 1) continue
    return {
      condition_text: `Список заданий викторины состоял из ${total} вопросов. За каждый правильный ` +
        `ответ ученик получал ${a} ${plural(a, "очко", "очка", "очков")}, за неправильный ответ с него ` +
        `списывали ${b} очков, а при отсутствии ответа давали 0 очков. Сколько вопросов ученик, ` +
        `набравший ${S} ${plural(S, "очко", "очка", "очков")}, оставил без ответа?`,
      answer: ru(z),
    }
  }
  return { condition_text: `Список заданий викторины состоял из 55 вопросов. За каждый правильный ответ ученик получал 9 очков, за неправильный ответ с него списывали 17 очков, а при отсутствии ответа давали 0 очков. Сколько вопросов ученик, набравший 153 очка, оставил без ответа?`, answer: "38" }
}
...
export const GENERATORS_EGE_BASE = {
  ...
  20: [t20MixSolutions, t20AddWater, t20WorkTogether, t20AvgThirds, t20AvgThereBack, t20Meeting,
    t20Alloy, t20Deposit, t20UpDown, t20Boat, t20Test, t20PedCyclist],
  21: [t21Snail, t21Well, t21AvgFifth, t21Boards, t21Grasshopper, t21Mushrooms, t21Tape,
    t21Table, t21FallenPages, t21Quiz],
}
...
export const GEN_META_EGE_BASE = {
  ...
  20: [["Растворы и сплавы", [
    ...
    ["alloy", "Два сплава (никель)", t20Alloy],
  ]],
    ["Работа и проценты", [
      ["work", "Совместная работа", t20WorkTogether],
      ["test", "Одинаковый тест", t20Test],
      ["deposit", "Банковский вклад", t20Deposit],
    ]],
    ["Движение", [
      ...
      ["ped-cyclist", "Пешеход и велосипедист", t20PedCyclist],
      ["up-down", "Дорога подъём-спуск", t20UpDown],
      ["boat", "Лодка и течение", t20Boat],
    ]]],
  21: [["Логика и последовательности", [
    ["snail", "Улитка на дереве", t21Snail],
    ["well", "Колодец (прогрессия)", t21Well],
    ["avg-fifth", "Среднее (n+1)-го числа", t21AvgFifth],
    ["boards", "Распил досок", t21Boards],
    ["grasshopper", "Кузнечик", t21Grasshopper],
  ]],
    ["Комбинаторика и числа", [
      ["mushrooms", "Грибы (рыжики/грузди)", t21Mushrooms],
      ["tape", "Лента с полосками", t21Tape],
      ["table", "Таблица (число строк)", t21Table],
      ["fallen-pages", "Выпавшие листы", t21FallenPages],
      ["quiz", "Викторина (очки)", t21Quiz],
    ]]],
}
