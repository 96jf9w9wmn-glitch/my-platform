...
import { examComposition } from "../examScales"
...
const EXAM_TIME = {
  ...
  // Информатика: время экзамена ЦЕЛИКОМ, хотя в лист входит только та его
  // часть, что решается без компьютера. Так и в демоверсии ФИПИ, и так же
  // считает таймер варианта (pages/examTiming.js) — печатать здесь меньшее
  // число значило бы разойтись с тем, что ученик видит на экране.
...
const isInformatics = (examType) => String(examType || "").includes("Информатика")

// Чем этот лист отличается от полного КИМ. Номера перечисляем: ученику важно не
// только «заданий меньше», но и каких именно нет — по ним он готовится отдельно.
// У информатики причина одна и та же: задание без компьютера не решается (к нему
// прилагается таблица, текстовый файл или архив), и на бумаге его дать нельзя.
function sheetNote(examType, total, numbers) {
...
// Первый абзац описывает НАСТОЯЩИЙ экзамен (examComposition), а не наш лист:
// в КИМ он именно такой, и число заданий там — проверяемый факт. До 01.09.2026
// абзац собирался по составу листа, и на варианте КЕГЭ печаталось «работа
...
function instructionParas({ examType, total, p1, p2, numbers = [] }) {
  ...
  const out = []
  const real = examComposition(examType)
  const exam = real.total ? real : { total, p1, p2 }
  out.push(exam.p2
    ? `Экзаменационная работа состоит из двух частей, включающих в себя ${exam.total} ${plu(exam.total, "задание", "задания", "заданий")}. Часть 1 содержит ${exam.p1} ${plu(exam.p1, "задание", "задания", "заданий")} с кратким ответом, часть 2 — ${exam.p2} ${plu(exam.p2, "задание", "задания", "заданий")} с развёрнутым ответом.`
  ...
  // дробный ответ записывается через запятую по указанию самого задания.
  out.push(inf
    ? `Ответом к каждому заданию является число или последовательность символов. Ответ запишите в поле «Ответ» после задания.`
    : `Ответом к каждому заданию${p2 ? " части 1" : ""} является число или последовательность цифр. Ответ запишите в поле «Ответ» после задания. Если получилась обыкновенная дробь, ответ запишите в виде десятичной.`)
  ...
  if (inf) {
    out.push("При выполнении заданий можно пользоваться черновиком. Записи в черновике не учитываются при оценивании работы.")
    out.push("Баллы, полученные Вами за выполненные задания, суммируются.")
  } else {
...
// Соглашения об обозначениях — отдельная страница КИМ по информатике, сразу за
// инструкцией (демоверсия ЕГЭ-2027, с. 4). Без неё лист непонятен: генераторы
// печатают связки ровно в том виде, в каком они напечатаны в банке ФИПИ
// (OPSTR в taskGeneratorsEgeInf.js), и расшифровать их ученику больше негде.
function conventionsParas(examType) {
  if (examType !== "ЕГЭ Информатика") return null
  return [
    "1. Обозначения для логических связок (операций): отрицание (инверсия, логическое НЕ) обозначается ¬ (например, ¬A); конъюнкция (логическое умножение, логическое И) обозначается /\\ (например, A /\\ B) либо & (например, A & B); дизъюнкция (логическое сложение, логическое ИЛИ) обозначается \\/ (например, A \\/ B) либо | (например, A | B); следование (импликация) обозначается → (например, A → B); тождество обозначается ≡ (например, A ≡ B). Выражение A ≡ B истинно тогда и только тогда, когда значения A и B совпадают. Символ 1 используется для обозначения истины, символ 0 — для обозначения лжи.",
    ...
    "3. Приоритеты логических операций: инверсия (отрицание), конъюнкция (логическое умножение), дизъюнкция (логическое сложение), импликация (следование), тождество. Таким образом, ¬A /\\ B \\/ C /\\ D означает то же, что и ((¬A) /\\ B) \\/ (C /\\ D). Возможна запись A /\\ B /\\ C вместо (A /\\ B) /\\ C; то же относится и к дизъюнкции.",
...
export async function generateVariantPdf({ title, examType, tasks, mode = "blank", part2Numbers = null }) {
  ...
  // Соглашения об обозначениях — сразу за инструкцией, как в КИМ по информатике.
  const conv = conventionsParas(examType)
  if (conv) push(
    `<div style="margin-bottom:6px;">В экзаменационных заданиях используются следующие соглашения.</div>` +
    conv.map((p) => `<div style="text-align:justify; margin-bottom:5px;">${escapeHtml(p)}</div>`).join(""))
