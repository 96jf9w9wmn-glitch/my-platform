...
//
// Файл специально сделан «чистым» JS без React (единственный импорт — num.js,
// такой же чистый): его подключают и клиент (страница «Подписка», ограничения
// в интерфейсе), и серверные функции в api/ (проверка права на ИИ-генерацию
// и онлайн-оплату). Разъезд цен или лимитов между клиентом и сервером — самый
// дорогой баг в биллинге, поэтому цифры живут ровно в одном месте.
//
...
export function formatPrice(rub) {
  return rub === 0 ? "Бесплатно" : `${fmtNum(rub)} ₽`
}
