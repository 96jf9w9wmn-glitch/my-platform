...
// Мягкий потолок ИИ-генераций для тарифов «без ограничений». Живой репетитор
// делает десятки генераций в месяц, самый активный сценарий — сотни; 2000 в
// месяц человеку не набрать. Потолок нужен не против пользователя, а против
// скрипта и утёкшего токена: DeepSeek считает деньги за каждый запрос, а
// единственной защитой иначе остаётся rate-limit по адресу (8/мин — это до
// 345 000 генераций в месяц). В интерфейсе тариф честно остаётся безлимитным:
// упереться в этот предел обычным пользованием невозможно.
