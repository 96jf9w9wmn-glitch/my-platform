...
// Время из кадра репетитора в тот кадр, в котором ученик это УВИДИТ, — для
// любого текста, который он прочитает (уведомление о переносе, пометка «не
// состоялось»). Это пояс его устройства, а не якорь: якорь говорит лишь, в
// каком поясе время записано, а кабинет ученика показывает его по своим часам.
// Пояса устройства нет (ученик ни разу не входил) — остаётся якорь.
export function toStudentWall(student, dateStr, timeStr) {
  return convertWall(dateStr, timeStr, student?.tzFrame, student?.accountTimezone || student?.timezone)
}

// Время из кадра репетитора в ЯКОРЬ — то есть в тот вид, в котором оно ложится
// в базу. Нужно там, где мы сами собираем хранимое значение (строка-витрина
// расписания); занятия переводит App.jsx при сохранении карточки.
export function toStoredWall(student, dateStr, timeStr) {
