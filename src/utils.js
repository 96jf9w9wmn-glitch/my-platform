...
function dataTableHtml(body) {
  ...
  // Считаем ВИДИМУЮ длину: в ячейке лежат и теги, и ещё не развёрнутые токены
  // (⟦i:F⟧ — курсивная F). Без снятия токенов таблица №2 КЕГЭ считалась «длинной»
  // и колонки расходились: пустая клетка выходила уже заполненной.
  const plain = (c) => c.replace(/<[^>]*>/g, "").replace(/⟦[a-zA-Z]+:?/g, "").replace(/⟧/g, "").trim()
...
export function plainTaskMath(text) {
  ...
  return String(text)
    ...
    // Пробелы по краям обязательны: в разметке листинг — отдельный блок и вплотную
    // к тексту не липнет, а в плоской строке без них выходит «Редактора:НАЧАЛО» и
    // «КОНЕЦНа вход приведённой выше программе».
    .replace(/⟦code:([^⟧]*)⟧/g, " $1 ")
...
export function renderHomeworkMath(text) {
  ...
  s = latexSymbols(s)
    ...
    // Одиночный индекс без скобок: x_1, a_n, 1010_2 (групповые ^{…}/_{…} уже
    // развёрнуты выше). Подчёркивание бывает и в обычном тексте работы («ДЗ_#3»,
    // «#1_Задание 1»), поэтому правило узкое, иначе буква после подчёркивания
    // молча падала в индекс и слово рассыпалось: слева — латинская буква или
    // цифра (основание формулы, а не хвост русского слова), справа — цифры или
    // ОДНА буква, за которой слово не продолжается.
    .replace(/([0-9A-Za-z])_(\d+|[A-Za-zА-Яа-яЁё])(?![0-9A-Za-zА-Яа-яЁё_])/g,
      (_, base, x) => `${base}<sub class="tmath-sub">${x}</sub>`)
...
// Раньше не проверялось вообще ничего: «+7 12» проходило и в регистрацию
// ученика, и в карточку у репетитора. Для ученика телефон — это ЛОГИН и
// единственная связка карточки с аккаунтом (по нему сшивает и RLS), поэтому
// кривой номер означает не косметику, а потерянный доступ и вторую карточку на
// того же человека.
//
// Формат один — российский: маска в форме жёстко «+7 (XXX) XXX-XX-XX», и
// притворяться, что принимаем что-то ещё, честнее не начинать.
// Первая цифра после кода 7 — от 3 до 9: коды 0xx, 1xx и 2xx в российской
// нумерации не выданы, а «+7 000…» проходило бы любую проверку на длину.
export function isValidRuPhone(raw) {
  const s = String(raw || "").trim()
  const d = s.replace(/\D/g, "")
  let body = null
  // Код страны указан явно («+7…», «8…») — номер обязан быть полным. Без этой
  // ветки «+7 912 345 67 8» (девять цифр вместо десяти) сходило бы за номер
  // без кода страны: форма ведь сама подставляет «+7», и первая семёрка
  // читалась бы как начало номера.
  if (/^[+8]/.test(s)) {
    if (d.length === 11 && (d[0] === "7" || d[0] === "8")) body = d.slice(1)
  } else if (d.length === 11 && (d[0] === "7" || d[0] === "8")) body = d.slice(1)
  else if (d.length === 10) body = d
  return !!body && /^[3-9]\d{9}$/.test(body)
}

// Почта проверяется по форме: доменная зона из букв, ровно одна собака, без
// пробелов и точек подряд. Существует ли ящик, формой не выяснить — это
// проверяет код подтверждения при регистрации репетитора (api/auth-signup.js).
export function isValidEmail(raw) {
  const v = String(raw || "").trim()
  if (v.length < 6 || v.length > 254 || /\s/.test(v) || v.includes("..")) return false
  return /^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/.test(v)
}

// Имя: «35235» именем не является. Требуем хотя бы две буквы — этого хватает,
// чтобы отсечь цифры и случайный набор, и при этом не мешает ни «Ян», ни
// «Аня 2» (так репетиторы различают двух тёзок), ни фамилии через дефис.
// Длину сверху ограничиваем, чтобы в поле не уехал текст целой страницы.
export function isValidPersonName(raw) {
  const v = String(raw || "").trim()
  if (v.length < 2 || v.length > 60) return false
  return (v.match(/[A-Za-zА-Яа-яЁё]/g) || []).length >= 2
}
...
// Условие в одну строку: описание домашней работы разбирается по строкам
// («1. …», «2. …»), и перенос внутри задания превратил бы его в два.
...
//
// Общая с формой выдачи ДЗ: разойдутся — и работа, собранная одной дорогой,
// будет проверяться автоматически, а точно такая же, собранная другой, нет.
const ANSWER_MAX = 40
export const isSimpleAnswer = (a) => a.length > 0 && a.length <= ANSWER_MAX && !/[\n\\{}]/.test(a)
