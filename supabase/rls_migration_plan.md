...
> ## ⚠️ ВЫПОЛНЕНО 30.07.2026, но ДРУГИМ СПОСОБОМ
>
> Этот документ оставлен как история решения. Реализация пошла не по нему:
> вместо перевода каждой ученической операции в SECURITY DEFINER RPC (несколько
> десятков функций плюс переписывание клиента) ученику и родителю выдаётся
> настоящий JWT, подписанный тем же секретом, которым PostgREST проверяет
> токены. Они ходят под ролью `app_user`, RLS работает нативно, а 235
> клиентских запросов остались без изменений.
>
> Что применено:
> - `rls_step1_secrets.sql` — закрыты `password_hash` и `session_token`
> - `rls_step2_identity.sql` — роль `app_user`, выдача токенов, `parent_login`
> - `rls_step3_policies.sql` — сами политики вместо «using (true)»
> - `rls_step4_tokens.sql` — RPC входа возвращают JWT
>
> Итог живых проб на боевой базе: аноним получает `42501` на всех таблицах с
> ПДн, ученик видит только свою карточку/ДЗ/переписку, родитель — только
> карточку ребёнка, репетитор — всё своё. Заявки с лендинга по-прежнему
> отправляются анонимом, но читать их он не может.
